"""
GigaPath Engine - Training/validation engine adapted for coordinate inputs.

Based on ABMIL engine but modified to handle (ID, Slide, Coords, Label) tuples.
"""
...
class Engine(object):
    ...
    def train(self, data_loader, model, criterion, optimizer):
        """Train one epoch - handles (ID, Slide, Coords, Label) tuples."""
        model.train()
        total_loss = 0.0
        ...
        # Modified: Unpack 4 values (ID, Slide, Coords, Label)
        for batch_idx, (data_ID, data_WSI, data_Coords, data_Label) in enumerate(dataloader):
            
            data_WSI = data_WSI.float()
            data_Coords = data_Coords.float()
            
            data_WSI = data_WSI.to(self.device)
            data_Coords = data_Coords.to(self.device)
            data_Label = F.one_hot(data_Label, num_classes=self.args.num_classes).float().to(self.device)
            
            # Modified: Pass both features and coordinates to model
            logit = model(data_WSI, data_Coords)
    ...
    def validate(self, data_loader, model, criterion, status="val"):
        """Validate/test - handles (ID, Slide, Coords, Label) tuples."""
        model.eval()
        total_loss = 0.0
        all_logits = np.empty((0, self.args.num_classes))
        all_labels = np.empty((0, self.args.num_classes))
    ...
    def metrics(self, logits, labels):
        ...
        # Compute NPV from StatScores
        stat_scores = general_results["StatScores"]
