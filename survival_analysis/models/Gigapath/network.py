...
class GigapathClassifier(nn.Module):
    ...
    def forward(self, features, coords, return_attn=False):
        """
        Forward pass through the GigaPath model for survival prediction.
        ...
        # Encoder requires autocast (FlashAttention needs fp16/bf16)
        with autocast():
        ...
        # Keep classifier + survival output in float32 (avoids backward dtype mismatch)
        slide_embedding = embeddings[-1].float()
